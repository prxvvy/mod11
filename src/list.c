...
Bool InsertAtBeginning(List *p_list, void *p_value, size_t valueSize) {
    ...
    p_newNode->p_next = p_list->p_head; /* Next pointer of new node points to the current head of the list. */
    p_list->p_head->p_previous = p_newNode;
    p_list->p_head = p_newNode; /* Update the head of the list. */
    p_list->p_tail->p_next = p_list->p_head; /* The tail of the list is still pointing to the old head so update the next pointer of the tail to new the head. */
...
Bool FirstEntry(List *p_list, Node *p_node) {
    /* Have the new node be the new head and tail of the list because it's empty. */
    p_list->p_head = p_node;
    p_list->p_tail = p_node;

    /* We know as for now, it's the only node in the list, so we do this. */
    p_list->p_head->p_previous = p_list->p_tail; /* Have it point itself as, currently, it's the only node therefore, head and tail should point each other */
    p_list->p_tail->p_next = p_list->p_head;
